                 ...
                 const OptionDef *options)
{
    ...
    LOGD("parse_option() check if default...");
    if (!po->name)
        po = find_option(options, "default");

    LOGD("parse_option() check Unrecognized...");
    if (!po->name) {
        LOGE("Unrecognized option '%s'\n", opt);
        return AVERROR(EINVAL);
    }
    LOGD("parse_option() check missing arguments...");
    ...
    if (po->flags & OPT_STRING) {
        ...
        *(double *)dst = parse_number_or_die(opt, arg, OPT_DOUBLE, -INFINITY, INFINITY);
        ...
        if (ret < 0) {
            LOGE("Failed to set value '%s' for option '%s'\n", arg, opt);
            return ret;
        }
    }
    LOGD("parse_option() checking....3");
    if (po->flags & OPT_EXIT){
        LOGE("Oh alright bye.");
        //exit_program(0);
    }

    LOGD("parse_option() checking....4");
